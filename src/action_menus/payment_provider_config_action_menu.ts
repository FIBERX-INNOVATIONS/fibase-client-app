...
class PaymentProviderConfigActionMenu {
    public static getMenus(
        ...
        const base_content_key = "content_resource.payment_provider_config_view_ui.list_view_ui.table.action_menu_list";
        const record_id = `${record.id ?? record.provider?.id ?? ""}${record.environment}`.toUpperCase();
...
export default PaymentProviderConfigActionMenu;
