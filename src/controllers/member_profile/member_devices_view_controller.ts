    ...
    implements MemberDevicesViewControllerInterface
{
    ...
    private readonly base_content_key =
        "content_resource.member_profile_view_ui.profile_view_ui.devices_view";
    ...
    private getContentObject(): MemberDevicesViewContentTextInterface {
        return {
            loading_text: this.getContent(
                `${this.base_content_key}.loading_text`,
                "Loading devices..."
            ),
            empty_state_text: this.getContent(
                `${this.base_content_key}.empty_state_text`,
                "No devices found"
            ),
            search_placeholder_text: this.getContent(
                `${this.base_content_key}.search_placeholder_text`,
                "Search devices"
            ),
            logout_all_button_text: this.getContent(
                `${this.base_content_key}.logout_all_button_text`,
                "Log All Devices Out"
            ),
            ...
            logged_out_button_text: this.getContent(
                `${this.base_content_key}.logged_out_button_text`,
                "Logged Out"
            ),
            current_device_badge_text: this.getContent(
                `${this.base_content_key}.current_device_badge_text`,
                "Current Device"
            ),
            ...
            empty_value_text: this.getContent(
                "content_resource.member_profile_view_ui.profile_view_ui.empty_value_text",
                "-"
            ),
            labels: {
                device_id: this.getContent(
                    `${this.base_content_key}.labels.device_id`,
                    "Device ID"
                ),
                ip_address: this.getContent(
                    `${this.base_content_key}.labels.ip_address`,
                    "IP Address"
                ),
                origin_url: this.getContent(
                    `${this.base_content_key}.labels.origin_url`,
                    "Origin URL"
                ),
                location_info: this.getContent(
                    `${this.base_content_key}.labels.location_info`,
                    "Location"
                ),
                is_2fa_validated: this.getContent(
                    `${this.base_content_key}.labels.is_2fa_validated`,
                    "2FA Validated"
                ),
                created_at: this.getContent(
                    `${this.base_content_key}.labels.created_at`,
                    "Created"
                ),
                logout_at: this.getContent(
                    `${this.base_content_key}.labels.logout_at`,
                    "Logged Out"
                ),
    ...
    protected getUIComputedData(): ComputedDefinitionType<MemberDevicesViewComputedDataInterface> {
        return {
            is_action_processing: () =>
                !!this.state_refs.processing_session_id.value ||
                this.state_refs.is_logging_all_out.value,

            can_logout_device: () =>
                MemberAuthenticatorUtil.memberHasPermissionTo(
                    "member_device_module.logout_member_device_login_session"
                ),

            can_logout_all_devices: () =>
                MemberAuthenticatorUtil.memberHasPermissionTo(
                    "member_device_module.logout_all_member_device_login_sessions"
                ),
    ...
    private formatBoolean(value?: boolean): string {
        return value
            ? this.state_refs.content_obj.value.bool_text.yes
            : this.state_refs.content_obj.value.bool_text.no;
    }
    ...
    private buildCurrentDeviceBadge(): string {
        return [
            ...
            `${this.escapeHtml(this.state_refs.content_obj.value.current_device_badge_text)}`,
            `</span>`
        ].join("");
    }
    ...
    private getDeviceDescription(device: MemberDeviceSessionViewRecordInterface): string {
        ...
        return [
            ...
            this.buildDescriptionItem(
                content_obj.labels.location_info,
                this.formatLocationInfo(device.location_info)
            ),
            this.buildDescriptionItem(
                content_obj.labels.is_2fa_validated,
                this.formatBoolean(device_2fa_validated)
            ),
            this.buildDescriptionItem(
                content_obj.labels.created_at,
                this.formatDateTime(device.created_at)
            ),
            this.buildDescriptionItem(
                content_obj.labels.logout_at,
                this.formatDateTime(device.logout_at)
            ),
            this.buildDescriptionItem(
                content_obj.labels.status,
                this.formatBoolean(device.is_active)
            )
        ].join(" ");
    }

    public getDeviceCardProps(
        ...
        return ContentCardUIPropsBuilder.getReactivePropsObject(
            `MemberDevice${device.id || device.device_id}`,
            {
                content_props: {
                ...
                boolean_props: {
                    disabled:
                        is_logged_out || this.state_refs.processing_session_id.value === device.id
                },
                action_props: {
